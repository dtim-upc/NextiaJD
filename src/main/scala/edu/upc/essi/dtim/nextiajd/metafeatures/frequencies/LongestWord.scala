...
object LongestWord extends MetaFeature with DefaultMF {
  val name: String = "longWord"
  val nameAtt: String = "len_max_word"
  ...
  val doc: String = "The number of characters in the longest value in the column"

  def func(columnName: String): Column = length(max(s"${columnName}_raw")).as(s"${columnName}_${nameAtt}")
}
